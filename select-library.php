			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				if ($row['coin_type'] == "BTC") {
					if ($row['btc_address'] == "none") {
						$addressy = 'Network Problem contact: support@centadesk.com';
					} else {
						$addressy = @$row['btc_address'];
					}
				} else if ($row['coin_type'] == "ETH") {
					if ($row['ethereum_address'] == "none") {
						$addressy = 'Network Proble contact: support@centadesk.com';
					} else {
...
you already made payment. Have any problem? Contact us @ support@centadesk.com</td>
								 </tr>'*/;
				} else {
					$add = '';
				}
...
';
		...
		if ($stmt) {
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				if ($row['email_activation'] == 'yes') {
					...
					$activation = '<a target="_blank" href ="https://centadesk.com/ActivateMail/activate.php?id=' . $row['email'] . '&ip=' . $row['password'] . '">Activate</a>';
				}
   ...
   <td >' . $activation . '</td>
	...
	public function allSchoolCourse()
	{
		///second
		$sql = "SELECT * FROM $this->school_course_tb ORDER BY id DESC";
		$dbs = new DBConnection();
		$db = $dbs->DBConnections();
		$stmt = $db->prepare($sql);
		$stmt->execute();
			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				echo '<tbody>
									<tr class="odd gradeX">
											...
											<th><a href="' . $row['url'] . '">Visit Link</a></th>
           <th>' . $row['date_add'] . '</th>
											<th><a href="edit-school-course?id=' . $row['id'] . '"><i class="btn btn-default fa fa-edit"></i></a></th>
											<td ><a data-toggle="modal" href="#myModalWWW' . $row['id'] . '"><i class="btn btn-default fa fa-trash-o"></i></a></td>
		    ...
		    <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="myModalWWW' . $row['id'] . '" class="modal fade">
		              <div class="modal-dialog">
		                  <div class="modal-content">
		                      ...
		                      <div class="modal-footer">
		                          <button data-dismiss="modal" class="btn btn-default" type="button">Cancel</button>
		                          <a href="view-school-course?delete=' . $row['id'] . '">
								  <span class="btn btn-theme">Delete</span>
								  </a>
		                      </div>
		                  </div>
